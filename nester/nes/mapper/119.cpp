...
void NES_mapper119::Reset()
{
  ...
  attached_ppu->vram_write_protect = 0;
}
...
void NES_mapper119::MMC3_set_PPU_banks()
{
  ...
  if(chr_bank[0] & 0x40)
    set_VRAM_bank0(0, chr_bank[0] & 0x07);
  else
    set_PPU_bank0(chr_bank[0]);

  if(chr_bank[1] & 0x40)
    set_VRAM_bank0(1, chr_bank[1] & 0x07);
  else
    set_PPU_bank1(chr_bank[1]);

  if(chr_bank[2] & 0x40)
    set_VRAM_bank0(2, chr_bank[2] & 0x07);
  else
    set_PPU_bank2(chr_bank[2]);

  if(chr_bank[3] & 0x40)
    set_VRAM_bank0(3, chr_bank[3] & 0x07);
  else
    set_PPU_bank3(chr_bank[3]);

  if(chr_bank[4] & 0x40)
    set_VRAM_bank0(4, chr_bank[4] & 0x07);
  else
    set_PPU_bank4(chr_bank[4]);

  if(chr_bank[5] & 0x40)
    set_VRAM_bank0(5, chr_bank[5] & 0x07);
  else
    set_PPU_bank5(chr_bank[5]);

  if(chr_bank[6] & 0x40)
    set_VRAM_bank0(6, chr_bank[6] & 0x07);
  else
    set_PPU_bank6(chr_bank[6]);

  if(chr_bank[7] & 0x40)
    set_VRAM_bank0(7, chr_bank[7] & 0x07);
  else
